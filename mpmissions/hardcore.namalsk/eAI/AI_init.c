...
void InitDynamicPatrols() {
	for (int i = 0; i < patrol_list.Count(); i++) {
		...
		int randomNumber = Math.RandomInt(MIN_PER_PATROL,MAX_PER_PATROL); // Take a random value between the values entered above, for each patrol.
		autoptr eAIDynamicPatrol pat = new eAIDynamicPatrol(patrol_list[i][0], patrol_list[i], loadout, randomNumber, MINR, MAXR, DESPAWNR, RESPAWN_SECONDS);
		patrols.Insert(pat);
		pat.UpdateTriggers();
	}
}
